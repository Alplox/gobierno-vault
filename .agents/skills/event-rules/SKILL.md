---
name: event-rules
description: Reglas de creación y validación de eventos, checklist de 16 reglas, enforcement de wikilinks en prosa y ciclo TAREAS. Usa esta skill SIEMPRE al crear o editar eventos, tocar sources.yaml/entities.yaml, registrar TAREAS/PENDIENTES o cuando validate.mjs falle por wikilink/URL, incluso si no mencionan 'reglas'.
---

# Reglas para crear/modificar eventos

> Cuándo cargar: vas a crear o editar eventos, tocar `sources.yaml`/`entities.yaml`, o registrar/retomar tareas en `TAREAS/`. Para frontmatter y wikilinks ver `content-model.md`.
...
3. **`tema`:** IDs de `topics.yaml`; si falta, agregarlo.
4. **`etiquetas`:** strings libres.
5. **`impacto.colectivos`:** IDs de `colectivos.yaml`.
6. **`impacto.sectores`:** IDs de `sectores.yaml`.
7. **`relaciones`:** `tipo: id` sin extensión (ej. `20260720-1`). No duplicar bidireccional.
8. **Wikilinks en body:** siempre que menciones persona/org/fuente/cifra, usa wikilink. Ver detalle abajo.
9. **Fuentes:** agregar a `sources.yaml` si es nueva. ID `medio-YYYY-MM-DD-slug`. **Siempre inline**, nunca `## Referencias`.
...
13. **Prohibido notas de editor en body:** no dejar `Nota de verificación`, `ver TAREAS*`, `para seguimiento`, `pendiente de validación`, `pendiente el desenlace`, etc. El body solo contiene hechos; cross-refs `(ver evento X)` sí válidos (auto-enlazan). Lo pendiente va a `TAREAS/` con `⬜`/`🟡`. `scripts/validate.mjs` hace fallar el build si detecta el patrón. Excepción: eventos-tracker diseñados (ej. `20250822-1`).
...
Vive en `TAREAS/` (desde 15-ago-2026; antes monolito `TAREAS.md` 995 líneas). No hay `TAREAS.md` raíz ni archivo de completadas — lo hecho queda en `EVENTS_INDEX.md` + `git log`.
...
1. Crear evento siguiendo `TEMPLATE.md`
2. Mínimo 5 fuentes de medios distintos, nunca red social como fuente única
3. Eventos 2019-2021 (era Piñera) necesitan entidades nuevas en `entities.yaml`
