...
function personCard(p) {
  const cargo = p.cargo
    ? `<p class="flex items-center gap-1.5 text-xs text-base-content/70 font-medium mt-1"><svg class="h-3.5 w-3.5 text-base-content/40 shrink-0" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><rect x="2" y="7" width="20" height="14" rx="2"/><path d="M16 7V5a2 2 0 0 0-2-2h-4a2 2 0 0 0-2 2v2"/></svg><span>${esc(p.cargo)}</span></p>`
    : '';
  ...
  <div class="flex items-start gap-4">
    ...
    <div class="min-w-0 flex-1">
      <h3 transition:name="person-title-${p.id}" class="font-bold text-base-content text-base group-hover:text-primary transition-colors leading-snug">${esc(p.nombre)}</h3>
      ${cargo}
      ${org}
    </div>
